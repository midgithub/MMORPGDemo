...
public static class XLuaHelper
{
    ...
    // 说明：构建List类型
    private static Type MakeGenericListType(Type itemType)
    {
    ...
    // 说明：构建Action类型
    private static Type MakeGenericActionType(params Type[] paramTypes)
    {
    ...
    // 说明：构建GameFrameworkAction类型
    private static Type MakeGenericGameFrameworkActionType(params Type[] paramTypes)
    {
        if (paramTypes == null || paramTypes.Length == 0)
        {
            return typeof(GameFrameworkAction);
        }
        else if (paramTypes.Length == 1)
        {
            return typeof(GameFrameworkAction<>).MakeGenericType(paramTypes);
        }
        else if (paramTypes.Length == 2)
        {
            return typeof(GameFrameworkAction<,>).MakeGenericType(paramTypes);
    ...
    // 说明：构建EventHandler
    delegate Type MakeGenericEventHandlerType(params Type[] paramTypes);
    private static EventHandler InnerCreateDelegate(MakeGenericEventHandlerType del, object target, Type type, string methodName, params Type[] paramTypes)
    {
        if (target != null)
        {
            type = target.GetType();
        ...
        return Delegate.CreateDelegate(delegateType, target, methodInfo) as EventHandler;
    }
}
